...
def evaluation():
    ...
    if not all(key in request.files for key in ('modelQuestionAnswer', 'modelQuestion', 'studentAnswers')):
        return jsonify({"error": "Both model question paper and student answers are required"}), 400
